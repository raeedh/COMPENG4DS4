...
//    xSemaphoreGive(rc_hold_semaphore);
    ...
    //Create RC Task
    status = xTaskCreate(rcTask, "RC task", 200, (void*) rc_hold_semaphore, 2, NULL);
    if (status != pdPASS) {
...
void rcTask(void *pvParameters) {
    ...
    while (1) {
//        xSemaphoreTake(rc_hold_semaphore, (TickType_t) portMAX_DELAY);
        UART_ReadBlocking(RC_UART, ptr, 1);
        if (*ptr != 0x20)
            continue;
        UART_ReadBlocking(RC_UART, &ptr[1], sizeof(rc_values) - 1);
        if (rc_values.header == 0x4020) {
            // printf("Channel 1 = %d\t", rc_values.ch1); // used control joy stick motor input
